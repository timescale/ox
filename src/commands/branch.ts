...
import { getRepoInfo, generateBranchName, type RepoInfo } from "../services/git";
import { forkDatabase, type ForkResult } from "../services/db";
...
async function branchAction(prompt: string, options: BranchOptions): Promise<void> {
...
export const branchCommand = new Command("branch")
  .description("Create a feature branch with isolated DB fork and start agent sandbox")
  .argument("<prompt>", "Natural language description of the task")
  .option("-s, --service-id <id>", "Database service ID to fork (defaults to tiger's default)")
